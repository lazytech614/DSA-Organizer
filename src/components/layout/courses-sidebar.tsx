...
import { ChevronRight, ChevronDown, BookOpen, User, Crown, AlertTriangle, Zap, Trash, X, ChevronLeft } from 'lucide-react';
...
export function CoursesSidebar({ 
  ...
  const toggleSection = (section: string) => {
    if (collapsed) return; // Don't allow section toggling when collapsed
  ...
  return (
    <div className={`${collapsed && !isMobile ? 'w-16' : 'w-full'} h-full bg-gray-900 border-r border-gray-700 flex flex-col transition-all duration-300 ${
      isMobile ? (isVisible ? 'opacity-100' : 'opacity-0') : 'opacity-100'
    }`}>
      {/* Header with Slide-in Animation */}
      <div className={`border-b border-gray-700 ${isMobile ? 'p-4' : 'p-4'} transition-all duration-300 delay-75 h-[80px] sm:h-[90px] lg:h-[100px] flex-shrink-0 ${
        ...
        <div className="flex items-center justify-between h-full">
          ...
          <Button
            variant="ghost"
            size="sm"
            onClick={isMobile ? handleClose : handleToggleCollapse}
            className="text-gray-400 hover:text-white p-2 transition-all duration-200 flex-shrink-0"
            title={collapsed ? "Expand sidebar" : (isMobile ? "Close sidebar" : "Collapse sidebar")}
          >
            {isMobile ? (
              <X className="w-4 h-4" />
            ) : collapsed ? (
      ...
      {!collapsed && isSignedIn && !isLoading && (
      ...
      {/* Content Area */}
      <div className={`
        flex-1 overflow-y-auto ${collapsed && !isMobile ? 'p-2' : (isMobile ? 'p-3' : 'p-4')} 
        ...
        ${isMobile ? '[-webkit-overflow-scrolling:touch] overscroll-y-contain' : ''}
      `}>
        
        {collapsed && !isMobile ? (
          // Collapsed view - Icons only (Desktop)
          <div className="space-y-4">
            {/* Default Courses Icon */}
            <div className="flex justify-center">
              <Button
                variant="ghost"
                size="sm"
                onClick={() => onToggleCollapse && onToggleCollapse()}
                className="p-2 text-orange-400 hover:bg-orange-500/10 rounded-lg transition-all duration-200"
            ...
            {/* Course Icons */}
            <div className="space-y-2">
              {defaultCourses.slice(0, 3).map((course, index) => (
              ...
              {defaultCourses.length > 3 && (
                <div className="flex justify-center">
                  <div 
                    className="p-2 text-xs text-gray-500 bg-gray-800 rounded-lg"
                    title={`+${defaultCourses.length - 3} more default courses`}
                  >
                    +{defaultCourses.length - 3}
                  </div>
                </div>
              ...
              <>
                ...
                <div className="space-y-2">
                  ...
                  {userCourses.length > 3 && (
                    <div className="flex justify-center">
                      <div 
                        className="p-2 text-xs text-gray-500 bg-gray-800 rounded-lg"
                        title={`+${userCourses.length - 3} more user courses`}
                      >
                        +{userCourses.length - 3}
                      </div>
                    </div>
                ...
                <div className="flex justify-center">
                  <Button
                    ...
                    size="sm"
                    onClick={() => onToggleCollapse && onToggleCollapse()}
                    className="p-2 text-green-400 hover:bg-green-500/10 rounded-lg transition-all duration-200"
                    title="Add Course"
                  >
            ...
            {isSignedIn && userInfo && (
              <div className="flex justify-center mt-4">
                ...
                }`} title={userInfo.isPro ? 'Pro Member' : 'Free Plan'}>
                  {userInfo.isPro ? (
                    <Crown className="w-4 h-4" />
                  ) : (
                    <span className="text-xs font-bold">
                      {userInfo.limits.coursesUsed}/{userInfo.limits.maxCourses}
                    </span>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="transform transition-all duration-300">
              ...
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${
                expandedSections.includes('default') ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
              }`}>
            ...
            {isSignedIn ? (
              <div className="transform transition-all duration-300">
                ...
                <div className={`overflow-hidden transition-all duration-300 ease-in-out ${
                  expandedSections.includes('user') ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'
                }`}>
                  <div className={`mt-2 space-y-1 ${isMobile ? 'ml-4' : 'ml-6'} pr-1`}>
